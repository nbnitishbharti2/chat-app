...
class Helper{
	...
	getChatList(userId){
		try {
			return Promise.all([
				this.db.query(`SELECT users.id, users.name, users.socket_id, users.online, users.updated_at FROM users JOIN users_admin ON users_admin.admin_id = users.id WHERE users_admin.user_id = ?`, [userId])
			]).then( (response) => {
				return {
					chatlist : response[0]
				};
